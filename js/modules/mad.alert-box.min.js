var MadAlertBox = (function (b) {
  function d(a, c) {
      ...
      containerClass: {
        get: function () {
          return (
            ...
            ' ' +
            (this.config.cssPrefix + this.config.classMap[this.config.type])
          )
        },
      },
      innerClass: {
    ...
    this.$element = void 0 === c ? this._generateTemplate() : c
    this._bindEvents()
  }
  d.prototype._generateTemplate = function () {
    this.$alert = b('<div></div>', {
      role: 'alert',
      class: this.containerClass,
      style: 'display: none',
    })
    this.$alertInner = b('<div></div>', {
      class: this.innerClass,
      text: this.config.message,
    })
    this.config.hasClose &&
      ((this.$alertClose = b('<button></button>', {
        type: 'button',
  ...
  d.prototype._bindEvents = function () {
    ...
    if (this.config.hasClose)
      this.$elementClose.on('click.AlertBox', function (b) {
        a.close()
        b.preventDefault()
      })
  }
  d.prototype.render = function () {
    var a = this,
      c = this.config.anchor
    c.length || (c = b('body'))
    c.after(this.$element)
    setTimeout(function () {
      a.$element.slideDown({
        easing: a.config.easing,
        duration: a.config.duration,
        complete: function () {
          b(document).trigger('pushed.milenia.alert', [b(this), a])
        },
      })
    }, 4)
    return this.$element
  ...
  d.prototype.close = function () {
    ...
    this.$element.slideUp({
      ...
      complete: function () {
        b(this).remove()
        b(document).trigger('closed.milenia.alert', [b(this), a])
      },
    })
  }
  return {
    init: function (a, c) {
      return a.each(function (a, f) {
        var e = b(f)
        e.data('AlertBox', new d(c, e))
      })
    },
    push: function (a) {
      a = new d(a)
      a.$element.data('AlertBox', a)
      return a.render()
    },
  }
})(window.jQuery)
